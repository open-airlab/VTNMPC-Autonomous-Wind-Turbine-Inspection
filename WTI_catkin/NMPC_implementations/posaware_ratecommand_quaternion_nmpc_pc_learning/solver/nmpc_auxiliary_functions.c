...
{
	return nmpcVariables.x;
}

real_t* nmpc_getVariablesU( )
{
	return nmpcVariables.u;
}

#if NMPC_NY > 0
real_t* nmpc_getVariablesY( )
{
	return nmpcVariables.y;
}
#endif
...
{
	return nmpcVariables.yN;
}
#endif

real_t* nmpc_getVariablesX0( )
{
#if NMPC_INITIAL_VALUE_FIXED
	return nmpcVariables.x0;
#else
	return 0;
#endif
}
...
void nmpc_printDifferentialVariables( )
{
	...
	for (i = 0; i < NMPC_N + 1; ++i)
	{
		for (j = 0; j < NMPC_NX; ++j)
			printf("\t%e", nmpcVariables.x[i * NMPC_NX + j]);
		printf("\n");
	}
	printf("]\n\n");
}
...
void nmpc_printControlVariables( )
{
	...
	{
		for (j = 0; j < NMPC_NU; ++j)
			printf("\t%e", nmpcVariables.u[i * NMPC_NU + j]);
		printf("\n");
	}
	printf("]\n\n");
}
