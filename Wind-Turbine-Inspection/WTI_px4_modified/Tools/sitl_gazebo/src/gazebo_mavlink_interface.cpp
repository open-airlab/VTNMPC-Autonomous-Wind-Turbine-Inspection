...
void GazeboMavlinkInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
    ...
    getSdfParam<std::string>(
        _sdf, "motorSpeedCommandPubTopic", motor_velocity_reference_pub_topic_, motor_velocity_reference_pub_topic_);
    ...
    // Added by Mohit
    if (_sdf->HasElement("tiltrotorConfig"))
    {
        std::string tiltrotor_config = _sdf->GetElement("tiltrotorConfig")->Get<std::string>();
        if (tiltrotor_config == "yes")
            is_tiltrotor_config = 1;
        else if (tiltrotor_config == "no")
            is_tiltrotor_config = 0;
        else
            gzerr << "[gazebo_mavlink_interface] Please only use 'yes' or 'no' as "
                     "tiltrotorConfig.\n";
    }
    else
        is_tiltrotor_config = 0;

    gzmsg << "[gazebo_mavlink_interface] is_tiltrotor_config = " << is_tiltrotor_config << "\n";
            ...
            if (channel->HasElement("input_index"))
            {
                ...
                if (index < n_out_max)
                {
                    ...
                    if (joint_control_type_[index] == "position_gztopic")
                    {
                        ...
                        joint_control_pub_[index] = node_handle_->Advertise<gazebo::msgs::Any>(gztopic_[index]);
#else
                        joint_control_pub_[index] = node_handle_->Advertise<gazebo::msgs::GzString>(gztopic_[index]);
#endif
                    }
                    ...
                    if (channel->HasElement("joint_name"))
                    {
                        ...
                        {
                            gzwarn << "joint [" << joint_name << "] not found for channel[" << index
                                   << "] no joint control for this channel.\n";
                        }
                        else
                        {
                            gzdbg << "joint [" << joint_name << "] found for channel[" << index
                    ...
                    // std::cout<<"joint_control_type_["<<index<<"] =
    ...
    updateConnection_ =
        event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboMavlinkInterface::OnUpdate, this, _1));

    // Subscriber to IMU sensor_msgs::Imu Message and SITL message
    imu_sub_ =
        node_handle_->Subscribe("~/" + model_->GetName() + imu_sub_topic_, &GazeboMavlinkInterface::ImuCallback, this);
    lidar_sub_ = node_handle_->Subscribe(
        "~/" + model_->GetName() + lidar_sub_topic_, &GazeboMavlinkInterface::LidarCallback, this);
    opticalFlow_sub_ = node_handle_->Subscribe(
        "~/" + model_->GetName() + opticalFlow_sub_topic_, &GazeboMavlinkInterface::OpticalFlowCallback, this);
    sonar_sub_ = node_handle_->Subscribe(
        "~/" + model_->GetName() + sonar_sub_topic_, &GazeboMavlinkInterface::SonarCallback, this);
    irlock_sub_ = node_handle_->Subscribe(
        "~/" + model_->GetName() + irlock_sub_topic_, &GazeboMavlinkInterface::IRLockCallback, this);
    gps_sub_ =
        node_handle_->Subscribe("~/" + model_->GetName() + gps_sub_topic_, &GazeboMavlinkInterface::GpsCallback, this);
    groundtruth_sub_ = node_handle_->Subscribe(
        "~/" + model_->GetName() + groundtruth_sub_topic_, &GazeboMavlinkInterface::GroundtruthCallback, this);
    vision_sub_ = node_handle_->Subscribe(
        "~/" + model_->GetName() + vision_sub_topic_, &GazeboMavlinkInterface::VisionCallback, this);

    // Publish gazebo's motor_speed message
    motor_velocity_reference_pub_ = node_handle_->Advertise<mav_msgs::msgs::CommandMotorSpeed>(
...
void GazeboMavlinkInterface::ImuCallback(ImuPtr& imu_message)
{
    ...
    ignition::math::Quaterniond q_gr = ignition::math::Quaterniond(imu_message->orientation().w(),
    ...
    ignition::math::Vector3d mag_noise_b(0.01 * randn_(rand_), 0.01 * randn_(rand_), 0.01 * randn_(rand_));

    ignition::math::Vector3d accel_b =
        q_br.RotateVector(ignition::math::Vector3d(imu_message->linear_acceleration().x(),
    ...
    ignition::math::Vector3d gyro_b = q_br.RotateVector(ignition::math::Vector3d(
        imu_message->angular_velocity().x(), imu_message->angular_velocity().y(), imu_message->angular_velocity().z()));
    ...
    if (imu_update_interval_ != 0 && dt >= imu_update_interval_)
    {
        ...
        // if vehicle is a tailsitter the airspeed axis is different (z points from
        // nose to tail)
...
void GazeboMavlinkInterface::VisionCallback(OdomPtr& odom_message)
{
    ...
    ignition::math::Vector3d position = q_ng.RotateVector(ignition::math::Vector3d(
        odom_message->position().x(), odom_message->position().y(), odom_message->position().z()));

    // q_gr is the quaternion that represents a rotation from ENU earth/local
    // frame to XYZ body FLU frame
    ignition::math::Quaterniond q_gr = ignition::math::Quaterniond(odom_message->orientation().w(),
    ...
    ignition::math::Vector3d linear_velocity =
        q_ng.RotateVector(q_br.RotateVector(ignition::math::Vector3d(odom_message->linear_velocity().x(),
    ...
    ignition::math::Vector3d angular_velocity =
        q_br.RotateVector(ignition::math::Vector3d(odom_message->angular_velocity().x(),
                                                   odom_message->angular_velocity().y(),
                                                   odom_message->angular_velocity().z()));

    // Only sends ODOMETRY msgs if send_odometry is set and the protocol version
    // is 2.0
...
/*ssize_t GazeboMavlinkInterface::receive(void *_buf, const size_t _size,
   uint32_t _timeoutMs)
   {
   fd_set fds;
   struct timeval tv;
    ...
    switch (msg->msgid)
    {
        case MAVLINK_MSG_ID_HIL_ACTUATOR_CONTROLS:
            ...
            for (int i = 0; i < input_reference_.size(); i++)
            {
                if (armed)
                {
                    input_reference_[i] = (controls.controls[input_index_[i]] + input_offset_[i]) * input_scaling_[i] +
                                          zero_position_armed_[i];
                    // Added by Mohit
                    //        if (joints_[i])
                    //           gzerr << i << " : " << input_index_[i] << " : " <<
                    //           controls.controls[input_index_[i]] << " : " <<
                    //           input_reference_[i] << "\n";
                    //        if (i <= 3) {
                    //          if (i == 0)
                    //            std::cout <<
                    ...
                    //                    << "] = " << input_reference_[i] << "\n";
                    //        }
                    if (is_tiltrotor_config && i <= 3)
                        std::cout << "controls.controls[" << i << "] = " << controls.controls[input_index_[i]]
                                  << ", input_reference_[" << i << "] = " << input_reference_[i] << "\n";
                    if (is_tiltrotor_config && i == 3)
                        std::cout << "servo angle command = " << input_reference_[i] << "\n";
                }
                else
                {
