...
void GazeboMultirotorBasePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("tiltrotorConfig")) {
    ...
    if (tiltrotor_config == "yes")
    {
      ...
      if (_sdf->HasElement("FinalAssemblyLinkName"))
        tiltrotor_final_assembly_name = _sdf->GetElement("FinalAssemblyLinkName")->Get<std::string>();
      else
      {
        tiltrotor_final_assembly_name = "tail_assembly_link";
        gzwarn << "[gazebo_multirotor_base_plugin] Taking FinalAssemblyLinkName name as: tail_assembly_link.\n";
      }
    }
    else if (tiltrotor_config == "no")
      is_tiltrotor_config = 0;
    else
      gzerr << "[gazebo_multirotor_base_plugin] Please only use 'yes' or 'no' as tiltrotorConfig.\n";
  }
  else
      is_tiltrotor_config = 0;
  gzmsg<<"[gazebo_multirotor_base_plugin] is_tiltrotor_config = "<<is_tiltrotor_config<<"\n";
  gzmsg<<"[gazebo_multirotor_base_plugin] FinalAssemblyLinkName: "<<tiltrotor_final_assembly_name<<"\n";
  ...
  // Added by Mohit
  if (is_tiltrotor_config)
  {
      physics::LinkPtr tail_assembly_link_ = model_->GetLink(tiltrotor_final_assembly_name);
      physics::Link_V tail_child_links_ = tail_assembly_link_->GetChildJointsLinks();
      for (unsigned int i = 0; i < tail_child_links_.size(); i++) {
        std::string link_name = tail_child_links_[i]->GetScopedName();
        ...
        if (pos != link_name.npos) {
          std::string motor_number_str = link_name.substr(pos + 6);
          unsigned int motor_number = std::stoi(motor_number_str);
          std::string joint_name = tail_child_links_[i]->GetName() + "_joint";
          physics::JointPtr joint = this->model_->GetJoint(joint_name);
          motor_joints_.insert(MotorNumberToJointPair(motor_number, joint));
        }
      }
...
//    std::cout<<"motor_rot_vel = "<<motor_rot_vel<<"\n";
  }
