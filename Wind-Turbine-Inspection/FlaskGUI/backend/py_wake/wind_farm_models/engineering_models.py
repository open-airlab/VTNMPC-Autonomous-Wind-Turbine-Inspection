...
import numpy as np
from backend.py_wake.deficit_models import DeficitModel
from backend.py_wake.superposition_models import SuperpositionModel, LinearSum, WeightedSum
from backend.py_wake.wind_farm_models.wind_farm_model import WindFarmModel
from backend.py_wake.deflection_models.deflection_model import DeflectionModel
from backend.py_wake.utils.gradients import use_autograd_in, autograd
from backend.py_wake.rotor_avg_models.rotor_avg_model import RotorCenter, RotorAvgModel
from backend.py_wake.turbulence_models.turbulence_model import TurbulenceModel
from backend.py_wake.deficit_models.deficit_model import ConvectionDeficitModel, BlockageDeficitModel, WakeDeficitModel
from backend.py_wake.ground_models.ground_models import NoGround, GroundModel
from tqdm import tqdm

from backend.py_wake.wind_turbines import WindTurbine, WindTurbines

from backend.py_wake.utils.model_utils import check_model
from backend.py_wake.utils.functions import mean_deg
...
class EngineeringWindFarmModel(WindFarmModel):
    ...
    def _add_blockage(self, deficit, dw_ijlk, **kwargs):
        ...
        if self.blockage_deficitModel is None:
            ...
            blockage = self.groundModel(
                lambda **kwargs: self.rotorAvgModel(self.blockage_deficitModel.calc_blockage_deficit, **kwargs),
    ...
    def _calc_deficit(self, dw_ijlk, **kwargs):
        ...
        deficit = self.groundModel(
            lambda **kwargs: self.rotorAvgModel(self.wake_deficitModel.calc_deficit_downwind, **kwargs),
            dw_ijlk=dw_ijlk, **kwargs)
...
class PropagateDownwind(EngineeringWindFarmModel):
    ...
    def __init__(self, site, windTurbines, wake_deficitModel,
        ...
        """
        EngineeringWindFarmModel.__init__(self, site, windTurbines, wake_deficitModel, rotorAvgModel,
                                          superpositionModel,
...
def main():
    if __name__ == '__main__':
        from backend.py_wake.examples.data.iea37 import IEA37Site, IEA37_WindTurbines
        from backend.py_wake.deficit_models.selfsimilarity import SelfSimilarityDeficit
        from backend.py_wake.deficit_models.gaussian import ZongGaussianDeficit
        from backend.py_wake.turbulence_models.stf import STF2017TurbulenceModel
        from backend.py_wake.flow_map import XYGrid
        ...
        from backend.py_wake.deficit_models.noj import NOJDeficit
        from backend.py_wake.superposition_models import SquaredSum
        ...
        zongp_ss = PropagateDownwind(site, windTurbines, wake_deficitModel=ZongGaussianDeficit(),
                                     superpositionModel=WeightedSum(),
                                     turbulenceModel=STF2017TurbulenceModel())

        # Zong convection superposition
        zong_ss = All2AllIterative(site, windTurbines, wake_deficitModel=ZongGaussianDeficit(),
                                   superpositionModel=WeightedSum(),
                                   blockage_deficitModel=SelfSimilarityDeficit(),
                                   turbulenceModel=STF2017TurbulenceModel())
