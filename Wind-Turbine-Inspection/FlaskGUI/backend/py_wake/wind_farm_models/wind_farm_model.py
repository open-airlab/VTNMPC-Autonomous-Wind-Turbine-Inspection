...
from backend.py_wake.site._site import Site, UniformSite, UniformWeibullSite, LocalWind
from backend.py_wake.wind_turbines import WindTurbines
import numpy as np
from backend.py_wake.flow_map import FlowMap, HorizontalGrid, FlowBox, YZGrid, Grid, Points
import xarray as xr
from backend.py_wake.utils import xarray_utils, weibull  # register ilk function @UnusedImport
from numpy import newaxis as na
from backend.py_wake.utils.model_utils import check_model, fix_shape
from backend.py_wake.utils.xarray_utils import da2py
...
class SimulationResult(xr.Dataset):
    ...
    def load(filename, wfm):
        ...
        sim_res = SimulationResult(wfm, lw, type_i=ds.type.values, yaw_ilk=ds.yaw, tilt_ilk=ds.tilt,
                                   WS_eff_ilk=ds.WS_eff.ilk(), TI_eff_ilk=ds.TI_eff.ilk(), power_ilk=ds.Power,
                                   ct_ilk=ds.CT,
                                   wt_inputs={})
...
def main():
    if __name__ == '__main__':
        ...
        from backend.py_wake import IEA37SimpleBastankhahGaussian
