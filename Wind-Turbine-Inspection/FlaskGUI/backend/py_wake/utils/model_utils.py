...
def get_exclude_dict():
    from backend.py_wake.deficit_models.deficit_model import DeficitModel, ConvectionDeficitModel, WakeDeficitModel,\
        BlockageDeficitModel
    from backend.py_wake.rotor_avg_models.rotor_avg_model import RotorAvgModel, RotorCenter
    from backend.py_wake.wind_farm_models.wind_farm_model import WindFarmModel
    from backend.py_wake.wind_farm_models.engineering_models import EngineeringWindFarmModel, PropagateDownwind

    from backend.py_wake.superposition_models import SuperpositionModel, LinearSum
    ...
    from backend.py_wake.ground_models import GroundModel
    from backend.py_wake.deficit_models.noj import NOJDeficit
    from backend.py_wake.ground_models.ground_models import NoGround
    return {
...
def cls_in(A, cls_lst):
    ...
    pywake_path = str(Path(backend.py_wake.__file__).parent.resolve())
